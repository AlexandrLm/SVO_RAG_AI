...
def init_db():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        conn.commit()
    print("База данных для истории диалогов инициализирована.")
...
def prune_history(messages_to_keep: int = 14):
    """
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        for session_id in sessions:
            ...
            """, (session_id, messages_to_keep))
            total_deleted += cursor.rowcount
        
        conn.commit()

    if total_deleted > 0:
        print(f"Очистка истории: удалено {total_deleted} старых записей (оставлено по {messages_to_keep} в каждой сессии).")
